  ...
  <div class='container column'>
    <form class='card card-w30' @submit.prevent='onSubmit'>
      ...
      <div class='form-control'>
        ...
        <textarea id='value' rows='3' v-model='cvItemDescription'></textarea>
    ...
    <div class='card card-w70'>
      <app-nick-name v-if='cvItem==="title"'></app-nick-name>
      ...
      <h3 v-if='isObjectEmpty'>Добавьте первый блок, чтобы увидеть результат</h3>
    </div>
  </div>
  <div class='container'>
    <p>
...
export default {
  ...
  methods: {
    isFormValid() {
      ...
      if (this.cvItemDescription.length <= 3) {
        isValidValue = false;
      }
      return isValidValue;
    },
    onSubmit() {
      if (this.isFormValid()) {
        this.cvBlocks[this.cvItem] = {
          cvBlock: this.cvItem,
          cvBlockDescription: this.cvItemDescription,
        };
        this.error = false;
  ...
  computed: {
    isObjectEmpty() {
      return Object.keys(this.cvBlocks).length === 0;
    },
  },
  components: {
    AppNickName,
